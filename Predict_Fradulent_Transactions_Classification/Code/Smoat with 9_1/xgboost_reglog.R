...
confusionMatrix(as.factor(y.train), as.factor(train.pred), positive = '1')

val.probs <- predict(xgb, data.matrix(x.val)) %>% as.matrix
val.pred <- ifelse(val.probs > 0.5, 1, 0)
confusionMatrix(as.factor(y.val), as.factor(val.pred), positive = '1')

test.probs <- predict(xgb, data.matrix(test.dummy)) %>% as.matrix
test.pred <- ifelse(test.probs > 0.5, 1, 0)

final.xg <- cbind.data.frame('transaction_id'=test.id, 'target'=test.pred)
write.csv(final.xg, file = 'final_xg_logit.csv', row.names = F)
